...
const ANALYSIS_SECTIONS = REPORTS_ANALYSIS_SECTIONS;
...
// Compare-mode chart instances live in chart-instances.js
...
function renderAnalysisSectionContent(section, ctx, savingsRate, options = {}) {
    ...
    if (!force && isReportsSectionCached(section, ctx)) return;
    ...
    const finishRender = () => {
        try {
            ...
            markReportsSectionCached(section, ctx);
        } finally {
...
function invalidateAnalysisRenderCache(ctx) {
    if (!invalidateReportsContextCacheIfChanged(ctx)) return;
